...
export const background = definePluginElement<BackgroundProps, undefined>({
    ...
    schema: {
        ...
        tabs: [{ id: 'appearance', label: 'Appearance', groups: [{ id: 'appearance', label: 'Appearance', collapsed: false, properties: [
    ...
    render(props, _state, time) {
        ...
        if (!viewport) return [];
        return [new Rectangle(0, 0, viewport.width, viewport.height, { fillColor: applyOpacity(props.color, props.opacity) })];
    },
});
