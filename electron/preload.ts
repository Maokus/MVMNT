...
const api: MvmntDesktopApi = {
    documents: {
        ...
        chooseSaveAs: (request: DesktopSaveAsSelectionRequest) =>
            ipcRenderer.invoke('documents:choose-save-as', request),
        beginSave: (request: DesktopDocumentSaveBeginRequest) => ipcRenderer.invoke('documents:save-begin', request),
        writeSaveChunk: (request: DesktopDocumentSaveWriteRequest) =>
            ipcRenderer.invoke('documents:save-write', request),
        completeSave: (sessionId: string) => ipcRenderer.invoke('documents:save-complete', sessionId),
        abortSave: (sessionId: string) => ipcRenderer.invoke('documents:save-abort', sessionId),
    ...
    menu: {
        onCommand: (callback: (command: DesktopMenuCommand) => void) => subscribe('menu:command', callback),
    },
    lifecycle: {
        onCloseRequest: (callback: () => void) => subscribe('lifecycle:close-request', callback),
        completeCloseRequest: (result: CloseRequestResult) => ipcRenderer.send('lifecycle:close-complete', result),
    },
    app: {
    ...
    exports: {
        chooseDestination: (request: DesktopExportDestinationRequest) =>
            ipcRenderer.invoke('exports:choose-destination', request),
    ...
    background: {
        ...
        onUpdate: (callback: (update: DesktopBackgroundExportUpdate) => void) =>
            subscribe('background:update', callback),
    ...
    automation: {
        ...
        onRenderRequest: (callback: (request: DesktopRenderRequest) => void) =>
            subscribe('automation:render-request', callback),
        ...
        onDeepLink: (callback: (command: DesktopDeepLinkCommand) => void) =>
            subscribe('automation:deep-link', callback),
    },
};
