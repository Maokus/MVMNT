...
describe('PerspectiveCompositor', () => {
    it('skips an entirely off-viewport warp before allocating WebGL or a source surface', () => {
        const createCanvas = vi.fn(() => { throw new Error('must not allocate'); });
        const compositor = new PerspectiveCompositor({ createCanvas });
        compositor.beginFrame();
        expect(compositor.renderElement(rootAt(500), {} as CanvasRenderingContext2D, {
    ...
    it('returns a deterministic affine fallback signal when WebGL initialization fails', () => {
        ...
        compositor.beginFrame();
        expect(compositor.renderElement(rootAt(0), {} as CanvasRenderingContext2D, {
    ...
    it('reuses source surfaces and texture storage at a stable size', () => {
        ...
        compositor.beginFrame();
        expect(compositor.renderElement(rootAt(0), target, { canvas: { width: 200, height: 200 } as HTMLCanvasElement }, 0)).toBe(true);
        expect(compositor.renderElement(rootAt(0), target, { canvas: { width: 200, height: 200 } as HTMLCanvasElement }, 0)).toBe(true);
        compositor.endFrame();
    ...
    it('falls back during context loss and recompiles after restoration', () => {
        ...
        expect(compositor.renderElement(rootAt(0), target, { canvas: { width: 200, height: 200 } as HTMLCanvasElement }, 0)).toBe(false);
    ...
    it('falls back on upload/draw errors and caps source surfaces to texture limits', () => {
        ...
        expect(failed.compositor.renderElement(rootAt(0), failed.target, { canvas: { width: 200, height: 200 } as HTMLCanvasElement }, 0)).toBe(false);

        const limited = createWorkingHarness(64);
        limited.compositor.beginFrame();
        expect(limited.compositor.renderElement(rootAt(0), limited.target, { canvas: { width: 200, height: 200 } as HTMLCanvasElement }, 0)).toBe(true);
...
function createWorkingHarness(maxTextureSize = 2048) {
    const gl: any = {
        ...
        CLAMP_TO_EDGE: 13, UNPACK_FLIP_Y_WEBGL: 14, MAX_TEXTURE_SIZE: 15, UNPACK_PREMULTIPLY_ALPHA_WEBGL: 21,
        COLOR_BUFFER_BIT: 16, FLOAT: 17, RGBA: 18, UNSIGNED_BYTE: 19,
        TRIANGLES: 20, NO_ERROR: 0,
        ...
        createBuffer: vi.fn(() => ({})), createTexture: vi.fn(() => ({})), bindBuffer: vi.fn(),
        bufferData: vi.fn(), bindTexture: vi.fn(), texParameteri: vi.fn(), pixelStorei: vi.fn(),
        ...
        texImage2D: vi.fn(), texSubImage2D: vi.fn(), getUniformLocation: vi.fn(() => ({})),
        uniform1i: vi.fn(), uniformMatrix3fv: vi.fn(), uniform4f: vi.fn(), uniform1f: vi.fn(),
        uniform2f: vi.fn(), drawArrays: vi.fn(), getError: vi.fn(() => 0),
        deleteTexture: vi.fn(), deleteBuffer: vi.fn(), deleteProgram: vi.fn(),
    };
    const sourceContext = {
        setTransform: vi.fn(), clearRect: vi.fn(), save: vi.fn(), scale: vi.fn(),
        translate: vi.fn(), restore: vi.fn(),
    } as unknown as CanvasRenderingContext2D;
    const scratch: any = {
        width: 0, height: 0, listeners: {} as Record<string, (event: any) => void>,
        addEventListener(type: string, listener: (event: any) => void) { this.listeners[type] = listener; },
    ...
    const target = { save: vi.fn(), restore: vi.fn(), drawImage: vi.fn(), globalAlpha: 1, globalCompositeOperation: 'source-over' } as unknown as CanvasRenderingContext2D;
