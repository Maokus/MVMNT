            ...
            <div className="relative">
                ...
                <div className="relative flex w-[min(90vw,22rem)] flex-col items-center gap-5 overflow-hidden rounded-3xl border border-neutral-800/80 bg-neutral-900/85 px-8 py-9 text-center shadow-[0_35px_120px_-40px_rgba(79,70,229,0.55)] backdrop-blur">
                    <div className="relative flex h-12 w-12 items-center justify-center">
                        <span
                            className="absolute h-12 w-12 animate-ping rounded-full bg-indigo-400/25"
                            aria-hidden="true"
                        />
                        <span
                            className="relative h-12 w-12 animate-spin rounded-full border-[3px] border-indigo-300/60 border-t-transparent"
                            aria-hidden="true"
                        />
                    </div>
...
export function App() {
    ...
    useEffect(() => {
        ...
        void desktop.background.take().then((request) => {
            ...
            sessionStorage.setItem(
                'mvmnt.desktop.background-export.v1',
                JSON.stringify({
                    jobId: request.jobId,
                    kind: request.kind,
    ...
    useEffect(() => {
        ...
        return desktop.automation.onDeepLink((command) => {
            ...
            if (command.command === 'open-community')
                navigate(command.id ? `/community?id=${encodeURIComponent(command.id)}` : '/community');
        });
    }, [navigate]);
                        ...
                        if (file.category === 'image') useVisualAssetRegistryStore.getState().addAsset(browserFile);
                            ...
                            window.dispatchEvent(
                                new CustomEvent('mvmnt-dropped-media', {
                                    detail: { category: file.category, file: browserFile },
                                })
                            );
                    }
                }
