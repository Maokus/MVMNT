...
1. **Status chip copy & hover details**
    - Reword track status labels to match the flow (`Idle`, `Pending`, `Analyzed`, `Failed`). Reuse existing badge styling in `TrackLanes.tsx` while updating label strings, and extend hover content to enumerate missing/stale descriptors using diagnostics data.【F:src/workspace/panels/timeline/TrackLanes.tsx†L260-L338】【F:src/state/audioDiagnosticsStore.ts†L171-L360】
