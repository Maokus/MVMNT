...
export interface VectorscopePoint { x: number; y: number; age: number; level: number; }
...
export function buildVectorscopeRgbColors(
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        while (pointIndex < target && sourceIndexForPoint(pointIndex, count, target) === index) {
            colors[pointIndex] = `rgb(${rgbComponent(lowEnergy)}, ${rgbComponent(midEnergy)}, ${rgbComponent(highEnergy)})`;
            pointIndex += 1;
        }
    }
    return colors;
...
export class AudioVectorscopeElement extends SceneElement {
        ...
        return insertElementConfig(super.getConfigSchema(), { name: 'Audio Vectorscope', description: 'Stereo mid/side XY display drawn from raw audio.', category: 'Audio Displays' }, [
            tab.content([
                ...
                { id: 'vectorscope', label: 'Vectorscope', collapsed: false, properties: [
                    ...
                    { key: 'scale', type: 'number', label: 'Display Scale', default: 1, min: 0.05, max: 10, step: 0.05, runtime: { transform: boundedNumber(0.05, 10), defaultValue: 1 } },
                    ...
                    { key: 'traceMode', type: 'select', label: 'Trace Mode', default: 'lines', options: [{ label: 'Lines', value: 'lines' }, { label: 'Points', value: 'points' }], runtime: { transform: (value) => normalizeTraceMode(value), defaultValue: 'lines' } },
                    { key: 'colorMode', type: 'select', label: 'Point Color', default: 'solid', options: [{ label: 'Solid', value: 'solid' }, { label: 'RGB (Frequency Bands)', value: 'rgb-meter' }], runtime: { transform: (value) => normalizeColorMode(value), defaultValue: 'solid' } },
    ...
    protected override _buildRenderObjects(_config: unknown, targetTime: number): RenderObject[] {
        ...
        if (props.showGrid !== false) this.addGrid(objects, width, height, props.gridColor ?? DEFAULT_GRID_COLOR, props.gridOpacity ?? 0.5, props.showLabels !== false, mode, scale, gain);
        const points = buildVectorscopePoints(left.value, right, width, height, gain, clamp(Math.round(props.pointCount ?? 1024), 64, 2048), mode, scale);
        ...
        if (traceMode === 'points') {
            ...
            const rgbColors = colorMode === 'rgb-meter'
            ...
            for (const [index, point] of points.entries()) {
                ...
                const dot = new Rectangle(point.x - pointSize / 2, point.y - pointSize / 2, pointSize, pointSize, {
                    fillColor: applyOpacity(color, traceOpacity * (0.18 + 0.82 * point.age)), layoutParticipation: 'exclude',
                });
                ...
                dot.blendMode = colorMode === 'rgb-meter'
                    ? 'lighter'
    ...
    private addGrid(
        objects: RenderObject[], width: number, height: number, color: string, opacity: number, showLabels: boolean, mode: VectorscopeMode, scale: number, gain: number
    ): void {
        ...
        const addScaleMarkers = (centered: boolean) => {
            ...
            if (!markerValues.length) { addLabel(width - 4, 12, 'Gain 0', 'right'); return; }
        ...
        if (mode === 'unipolar-scaled' || mode === 'unipolar-unscaled') {
            ...
            if (showLabels) { addLabel(4, 12, 'Mid'); addLabel(width - 4, height - 16, 'Side', 'right'); }
