...
const CommunityEditModal: React.FC<CommunityEditModalProps> = ({
    item,
    user,
    canCreateTags = false,
    onClose,
    ...
    const inputClass =
        'w-full rounded border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-sm text-neutral-200 placeholder-neutral-500 focus:border-indigo-500 focus:outline-none';
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {detectedVersion && (
                                <p className="mt-1 text-xs text-neutral-500">
                                    Detected from manifest: {detectedVersion}
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        {thumbnailPreview ? (
                            <img
                                ...
                                alt="New thumbnail"
                                className="mt-2 max-h-32 rounded border border-neutral-700 object-contain"
                            />
                        ) : (
                            <img
                                src={getThumbnailUrl(item.thumbnail_path)}
                                alt="Current thumbnail"
                                className="mt-2 max-h-32 rounded border border-neutral-700 object-contain opacity-50"
                            />
                        )}
                    </div>
                    ...
                    <div>
                        <label className="block text-xs text-neutral-400 mb-1">
                            {item.type === 'plugin'
                                ? 'Plugin file (leave blank to keep current)'
                                : 'Template file (leave blank to keep current)'}
                        </label>
                        <input
                            ref={fileInputRef}
                            type="file"
                        ...
                        {pluginUidConflict && detectedUid && (
                            <p className="mt-1 text-xs text-red-400">
                                A plugin with ID <span className="font-mono">{detectedUid}</span> already exists in the
                                community. Plugin IDs must be globally unique.
                            </p>
                        )}
                    </div>
