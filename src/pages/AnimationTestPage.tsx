...
const AnimationTestPage: React.FC = () => {
    ...
    for (let i = 0; i < phases.length; i++) {
        if (effectiveTime < acc + phases[i].duration) {
            currentPhaseIndex = i;
            break;
        }
    ...
    const updatePhase = (i: number, patch: Partial<PhaseConfig>) =>
            ...
            <div style={{ display: 'flex', gap: 24 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                                <label className="field-label">
                                    ...
                                    <input
                                        ...
                                        value={p.duration}
                                        onChange={(e) =>
                                            updatePhase(i, { duration: Math.max(1, parseInt(e.target.value) || 0) })
                                        }
                                        style={{ width: 80 }}
                                    />
                                </label>
                                <label className="field-label">
                                    ...
                                    <select
                                        ...
                                        value={p.easing}
                                        onChange={(e) => updatePhase(i, { easing: e.target.value })}
                                    >
                                        {EASING_NAMES.map((n) => (
                ...
                <div style={{ flex: 2 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <button
                            className="px-3 py-1 border rounded cursor-pointer text-xs font-medium transition inline-flex items-center justify-center"
                            onClick={() => {
                                if (!playing) {
                                    setStartTime(performance.now() - effectiveTime);
                                }
                                setPlaying((p) => !p);
                            }}
                        >
                        ...
                        <div style={{ fontSize: 12 }}>
                            t = {Math.round(effectiveTime)} ms / {total} ms
                        </div>
                        <div style={{ fontSize: 12 }}>
                            Phase: {phase.name} ({currentPhaseIndex + 1}/{phases.length})
                        </div>
                        <div style={{ fontSize: 12 }}>
                            Phase progress raw {rawProgress.toFixed(3)} eased {eased.toFixed(3)}
                        </div>
                    </div>
                    <input
                        type="range"
                    ...
                    >
                        {phases
                            .reduce<{ start: number; el: React.ReactElement }[]>((accum, p, i) => {
                    ...
                    <div className="grid-inputs">
                        <label className="field-label">
                            ...
                            <input
                                ...
                                type="number"
                                value={canvasWidth}
                                onChange={(e) => setCanvasWidth(Math.max(50, parseInt(e.target.value) || 0))}
                            />
                        </label>
                        <label className="field-label">
                            Canvas H
                            <input
                                ...
                                type="number"
                                value={canvasHeight}
                                onChange={(e) => setCanvasHeight(Math.max(50, parseInt(e.target.value) || 0))}
                            />
                        </label>
                        <label className="field-label">
                            Block W
                            <input
                                ...
                                type="number"
                                value={blockWidth}
                                onChange={(e) => setBlockWidth(Math.max(1, parseInt(e.target.value) || 0))}
                            />
                        </label>
                        <label className="field-label">
                            Block H
                            <input
                                ...
                                type="number"
                                value={blockHeight}
                                onChange={(e) => setBlockHeight(Math.max(1, parseInt(e.target.value) || 0))}
                            />
                        </label>
                        <label className="field-label">
                            Color
                        ...
                        <label className="field-label">
                            ...
                            <input
                                ...
                                value={blockNote}
                                onChange={(e) =>
                                    setBlockNote(Math.max(0, Math.min(127, parseInt(e.target.value) || 0)))
                                }
                            />
                        </label>
                        <label className="field-label">
                            ...
                            <input
                                ...
                                value={blockVelocity}
                                onChange={(e) =>
                                    setBlockVelocity(Math.max(0, Math.min(127, parseInt(e.target.value) || 0)))
                                }
                            />
                        </label>
                        <label className="field-label">
                            ...
                            <input
                                ...
                                value={blockChannel}
                                onChange={(e) =>
                                    setBlockChannel(Math.max(0, Math.min(15, parseInt(e.target.value) || 0)))
                                }
                            />
                        </label>
                        <label className="field-label">
                            ...
                            <input
                                ...
                                step="0.1"
                                value={blockDuration}
                                onChange={(e) => setBlockDuration(Math.max(0.01, parseFloat(e.target.value) || 0))}
                            />
                        </label>
                    </div>
                    <canvas
                    ...
                    <div style={{ fontSize: 12, marginTop: 4, color: '#ccc' }}>
                        Phase: {notePhase} progress {noteProgress.toFixed(2)}
                    </div>
    ...
    return (
        <div
            style={{
                ...
                left: '50%',
                top: '50%',
                transform: `translate(-50%, -50%) translateX(${x}px) scale(${scale.toFixed(3)}) rotate(${rotation.toFixed(1)}deg)`,
                borderRadius: 8,
                boxShadow: '0 0 12px #4af8',
            }}
        />
    );
};
