...
const CommunityTagInput: React.FC<CommunityTagInputProps> = ({ tags, onChange, max = 5, canCreateTags = false }) => {
    ...
    const suggestions = allTags.filter((t) => t.name.includes(normalized) && !tags.includes(t.name)).slice(0, 8);
                ...
                <div className="absolute z-10 mt-1 w-full rounded border border-neutral-700 bg-neutral-900 shadow-lg max-h-48 overflow-y-auto">
                    ...
                    {options.map((opt, i) => (
                        ...
                        >
                            {opt.type === 'create' ? (
                                <span>
                                    Create "<span className="font-medium">{opt.name}</span>"
                                </span>
                            ) : (
                                opt.name
                            )}
