...
export function DesktopWorkspaceTools() {
    ...
    const restore = async (version: AutosaveVersionSummary) => {
        ...
        if (!bytes) {
            setMessage('That recovery version is no longer available.');
            return;
        }
        ...
        return (
            <Modal title="Recovery Versions" onClose={() => setPanel(null)}>
                <p className="mb-4 text-sm text-neutral-400">
                    Up to {AutosaveVersionStore.policy.maxVersionsPerProject} versions per project are kept for{' '}
                    {AutosaveVersionStore.policy.maxAgeDays} days.
                </p>
                {versions.length === 0 ? (
                    <p className="text-sm text-neutral-400">No recovery versions are available.</p>
                ) : (
                    <div className="space-y-2">
                        {versions.map((version) => (
                            <div
                                key={version.id}
                                className="flex items-center gap-3 rounded border border-neutral-800 bg-neutral-950/60 p-3"
                            >
                                <div className="min-w-0 flex-1">
                                    <div className="truncate text-sm font-medium">{version.documentName}</div>
                                    <div className="text-xs text-neutral-500">
                                        {new Date(version.savedAt).toLocaleString()} · {formatBytes(version.size)}
                                    </div>
                                </div>
                                <button
                                ...
                                <button
                                    className="rounded border border-rose-700 px-3 py-1 text-xs text-rose-300 hover:bg-rose-950"
                                    onClick={() => void AutosaveVersionStore.remove(version.id).then(refresh)}
                                >
                                    Delete
                                </button>
                            </div>
                ...
                {versions.length > 0 && (
                    <button
                        className="mt-4 rounded border border-rose-700 px-3 py-1.5 text-xs text-rose-300"
                        onClick={() => {
                            if (confirm('Delete all recovery versions?'))
                                void AutosaveVersionStore.clear().then(refresh);
                        }}
                    >
                        Delete all versions
                    </button>
    ...
    return (
        <Modal title="Storage & Caches" onClose={() => setPanel(null)}>
            <p className="mb-4 break-all text-xs text-neutral-500">
                Desktop data: {nativeStorage?.location ?? 'Browser application storage'}
            </p>
            <div className="space-y-2 text-sm">
                <div className="flex items-center justify-between rounded border border-neutral-800 p-3">
                    <span>Current recovery package</span>
                    <button
                        className="rounded border border-neutral-700 px-2 py-1 text-xs"
                        disabled={currentFileBytes === 0}
                        onClick={() => {
                            if (confirm('Delete the current recovery package? Saved project files are not affected.'))
                                void LocalFileStore.clear().then(refresh);
                        }}
                    >
                        Clean {formatBytes(currentFileBytes)}
                    </button>
                </div>
                <div className="flex items-center justify-between rounded border border-neutral-800 p-3">
                    <span>Autosave versions ({versions.length})</span>
                    <span>{formatBytes(autosaveBytes)}</span>
                </div>
                <div className="flex items-center justify-between rounded border border-neutral-800 p-3">
                    <span>Interrupted temporary exports</span>
                    <button
                        className="rounded border border-neutral-700 px-2 py-1 text-xs"
                        onClick={() =>
                            void window.mvmntDesktop?.storage.cleanup('temporary-exports').then(setNativeStorage)
                        }
                    >
                        Clean {formatBytes(nativeStorage?.temporaryExports.bytes ?? 0)}
                    </button>
                </div>
                <div className="flex items-center justify-between rounded border border-neutral-800 p-3">
                    <span>Old application/update cache</span>
                    <button
                        disabled={!nativeStorage?.updateCache.available}
                        className="rounded border border-neutral-700 px-2 py-1 text-xs disabled:opacity-40"
                        onClick={() => void window.mvmntDesktop?.storage.cleanup('update-cache').then(setNativeStorage)}
                    >
                        Clean {formatBytes(nativeStorage?.updateCache.bytes ?? 0)}
                    </button>
                </div>
                <div className="flex items-center justify-between rounded border border-neutral-800 p-3">
                    <span>Decoded audio and feature caches</span>
                    <button className="rounded border border-neutral-700 px-2 py-1 text-xs" onClick={clearMemoryCaches}>
                        Clear memory caches
                    </button>
                </div>
            </div>
