...
Host-adapted built-ins use `tab.*` from `schema-groups`. Do **not** create the Transform tab; `insertElementConfig` adds it automatically. Callback-native definitions use the builders exported by `@mvmnt-app/plugin-sdk`.
