...
const measureLineWidth = (text: string, font: string, spacing: number): number => {
    try {
        ...
        if (context) { context.font = font; return context.measureText(text).width + text.length * spacing; }
    } catch { /* deterministic estimate below */ }
    const size = Number(font.match(/(\d*\.?\d+)px/)?.[1] ?? 16);
    return text.length * (size * 0.6 + spacing);
};
const parseFont = (value: string) => { const [family = 'Inter', weight = '400'] = value.split('|'); return { family, weight }; };

interface Props extends Readonly<Record<string, unknown>> {
    readonly text: string; readonly lineSpacing: number; readonly color: string; readonly opacity: number;
    readonly blendMode: GlobalCompositeOperation; readonly fontFamily: string; readonly fontSize: number;
    readonly textAlign: CanvasTextAlign; readonly letterSpacing: number; readonly strokeColor: string;
    readonly strokeWidth: number; readonly showBackground: boolean; readonly backgroundColor: string;
    readonly backgroundOpacity: number; readonly backgroundPaddingX: number; readonly backgroundPaddingY: number;
...
export const textOverlay = definePluginElement<Props, undefined>({
    ...
    schema: { tabs: [
        { id: 'content', label: 'Content', groups: [{ id: 'textContent', label: 'Content', collapsed: false, properties: [
            { key: 'text', label: 'Text Content', type: 'longString', default: 'Sample Text' },
            { key: 'lineSpacing', label: 'Line Spacing (px)', type: 'number', default: 4, min: 0, max: 80, step: 1 },
        ] }] },
        { id: 'appearance', label: 'Appearance', groups: [
            ...
            { id: 'typography', label: 'Typography', collapsed: false, properties: [
                ...
                { key: 'textAlign', label: 'Alignment', type: 'select', default: 'center', options: [{ value: 'left', label: 'Left' }, { value: 'center', label: 'Center' }, { value: 'right', label: 'Right' }] },
            ...
            { id: 'container', label: 'Container', collapsed: true, properties: [
                ...
                { key: 'backgroundColor', label: 'Background Color', type: 'colorAlpha', default: '#000000' },
    ...
    render(props) {
        ...
        const selected = parseFont(props.fontFamily);
        const font = `${selected.weight} ${props.fontSize}px ${selected.family}, sans-serif`;
        ...
        lines.forEach((line, index) => {
            const item = new Text(0, startY + index * (props.fontSize + props.lineSpacing), line, font, {
                color: applyOpacity(props.color, props.opacity), align: props.textAlign, baseline: 'top',
            });
            item.letterSpacing = props.letterSpacing; item.blendMode = props.blendMode === 'source-over' ? null : props.blendMode;
        ...
        if (props.showBackground) {
            ...
            const bg = new Rectangle(x - props.backgroundPaddingX, startY - props.backgroundPaddingY, width + props.backgroundPaddingX * 2, totalHeight + props.backgroundPaddingY * 2, { fillColor: applyOpacity(props.backgroundColor, props.backgroundOpacity) });
