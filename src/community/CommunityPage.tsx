...
const CommunityPage: React.FC = () => {
    ...
    const selectClass =
        'rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-neutral-200 focus:border-indigo-500 focus:outline-none cursor-pointer';
        ...
        <div className="min-h-screen bg-neutral-800 text-neutral-200 px-6 py-10">
            <main className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                    ...
                    <div className="flex flex-col items-end gap-3">
                        <div className="flex gap-2">
                            <Link
                                to="/"
                                className="px-4 py-2 rounded bg-neutral-700 hover:bg-neutral-600 text-sm font-medium text-neutral-300"
                            >
                                Back to Home
                            </Link>
                            <Link
                                to="/workspace"
                                className="px-4 py-2 rounded bg-indigo-600 hover:bg-indigo-500 text-sm font-medium text-white"
                            >
                                Back to Workspace
                            </Link>
                        </div>
                ...
                <div className="mb-6 rounded-lg border border-yellow-600/40 bg-yellow-900/20 px-4 py-3 text-sm text-yellow-200">
                    <span className="font-semibold">Experimental feature.</span> The community hub runs on very limited
                    server resources, which may cause long loading times and patchy availability. If you'd like to see
                    the backend resources upgraded, consider{' '}
                    <Link to="/contribute" className="underline hover:text-yellow-100">
                        contributing
                    </Link>
                    .{' '}
                    <span className="font-semibold text-red-300">
                        Backend updates may cause data loss and file corruption — always keep backups of your projects.
                    </span>
                </div>
                    ...
                    <div className="relative">
                        ...
                        {showTagDropdown && (
                            <div className="absolute z-10 mt-1 w-48 rounded border border-neutral-700 bg-neutral-900 shadow-lg max-h-48 overflow-y-auto">
                                ...
                                {allTags.filter((t) => t.name.includes(tagSearch) && !selectedTags.includes(t.name))
                                    .length === 0 && (
                ...
                {userRole === 'admin' && <AdminTagPanel onTagsChanged={handleItemChanged} />}
