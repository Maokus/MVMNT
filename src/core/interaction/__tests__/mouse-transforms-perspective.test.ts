...
describe('perspective-aware mouse transforms', () => {
    ...
    it('solves a warped scale handle in post-warp local space', () => {
        ...
        const result = computeScaledTransform(
            fixedWorld.x + pointerDelta.x,
            fixedWorld.y + pointerDelta.y,
            {
                mode: 'scale-se',
                origScaleX: 1,
                origScaleY: 1,
                ...
                dragLocalPoint: { x: 200, y: 100 },
                geom: {
                    widthVec: { x: 1, y: 0 },
                    heightVec: { x: 0, y: 1 },
                    corners: { TL: fixedWorld, TR: fixedWorld, BR: fixedWorld, BL: fixedWorld },
                    mids: { MTop: fixedWorld, MRight: fixedWorld, MBottom: fixedWorld, MLeft: fixedWorld },
                    baseBounds: bounds,
                },
                origRotation: 0,
                origSkewX: 0,
