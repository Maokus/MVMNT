// Enhanced Base BoundSceneElement class with Property Binding System
import { EnhancedConfigSchema, PropertyDefinition } from '@core/scene/runtime/schema';
import type { SceneElementInstance } from './types';
import { prop } from '@core/scene/runtime/schema-builders';
...
import { calculateElementBounds } from './element-bounds';
import { ManagedElementResources } from './managed-element-resources';
import {
    getRuntimePropertyDescriptors,
...
export class BoundSceneElement implements SceneElementInstance {
    public type: string;
    ...
    private readonly _managedResources = new ManagedElementResources();
    ...
    protected bundledImage(filename: string): BundledSprite {
        ...
        return this._managedResources.track(sprite);
    }
    ...
    protected bundledSparrow(pngFilename: string, xmlFilename: string, defaultFps?: number): BundledSparrowHandle {
        ...
        return this._managedResources.track(handle);
    }
    ...
    dispose(): void {
        this.onDestroy();
        this._managedResources?.dispose();
    ...
    protected getSchemaProps<TAdditional extends PropertyDescriptorMap<this>>(
        ...
        const ctor = this.constructor as typeof BoundSceneElement;
        const typedBase = getRuntimePropertyDescriptors<this>(ctor);
    ...
    private _getCachedSceneElementBounds(
        ...
        const computed = calculateElementBounds(renderObjects, mode, this.id);
