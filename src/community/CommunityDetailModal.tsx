...
import { loadPlugin, upgradePlugin, unloadPlugin, supportsPluginApiRange } from '@core/scene/plugins';
...
function getCompatBadge(item: CommunityItem): CompatBadge | null {
    ...
    if (item.plugin_api_version != null && !supportsPluginApiRange(item.plugin_api_version)) {
...
const CommunityDetailModal: React.FC<CommunityDetailModalProps> = ({
    ...
    const hasUpdate = isInstalled && item.version != null && semverGt(item.version, installedPlugin!.manifest.version);
                        ...
                        {item.tags.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-1.5">
                                {item.tags.map((tag) => (
                                    <span
                                        key={tag}
                                        className="rounded bg-neutral-800 border border-neutral-700 px-2 py-0.5 text-[11px] text-neutral-400"
                                    >
                                        {tag}
                                    </span>
                                ))}
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        {item.type === 'template' && (
                            ...
                            >
                                ...
                                {actioning ? 'Opening...' : 'Open in Workspace'}
                            </button>
                        )}
