...
Declare `audio.features.read`, register the required feature for the definition or instance
lifecycle, and sample it during render:
...
export const spectrum = definePluginElement({
    ...
    load(context) {
        const result = context.audio!.requireFeatures([{ feature: 'spectrogram' }]);
...
The same capability must appear in the element's `plugin.json` entry. Requirements are scoped,
deduplicated, and removed on unload. Sampling is clip-aware: the enabled clip at the requested
timeline time chooses the source, and gaps return silence. A failed result is normal while analysis
...
Declare both `audio.calculators.register` and `audio.features.read`. Register from `load()` and then
require the produced feature:
...
export const element = definePluginElement({
    ...
    load(context) {
        ...
        const requirement = context.audio!.requireFeatures([
