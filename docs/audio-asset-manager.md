...
crash recovery to refer back to the original media when decoded PCM buffers are missing
or intentionally evicted.

This system is implemented by `src/persistence/audio-asset-store.ts` and referenced from
timeline import, scene import/export, and the crash recovery journal.
...
This lets the app evict decoded PCM while preserving the ability to decode again later
from original media.
...
- export/import/recovery code translates missing bytes into warnings or failed decoded
  state.
