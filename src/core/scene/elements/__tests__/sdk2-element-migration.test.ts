...
const builtIns = [
    elements.background, elements.basicShapes, elements.image, elements.progressDisplay, elements.textOverlay,
    elements.timeDisplay, elements.timeUnitPianoRoll, elements.movingNotesPianoRoll, elements.notesPlayedTracker,
    elements.notesPlayingDisplay, elements.chordEstimateDisplay, elements.ccMonitor, elements.audioSpectrum,
    elements.audioVolumeMeter, elements.audioWaveform, elements.audioPeaks, elements.audioLockedOscilloscope,
    elements.debug,
];
...
describe('SDK 2 element migration inventory', () => {
    ...
    it('keeps every authoring template on definition/context APIs only', () => {
        const templates = [audioReactive, basicShape, bundledImage, gridAtlas, atlasImage, simpleImage, midiNotes, minimal, textDisplay];
        expect(templates.every((definition) => definition.kind === 'mvmnt.plugin-element.v2')).toBe(true);

        for (const filename of ['audio-reactive.ts', 'basic-shape.ts', 'bundled-image.ts', 'grid-atlas.ts', 'image-atlas.ts', 'image-simple.ts', 'midi-notes.ts', 'minimal.ts', 'text-display.ts']) {
