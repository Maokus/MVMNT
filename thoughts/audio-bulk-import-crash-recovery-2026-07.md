...
- Restore missing buffers by re-decoding from `AudioAssetStore`.
- Represent cache entries as `ready`, `decoding`, or `failed` so UI can show recoverable state instead of crashing.
...
- Add developer overlay rows for ready and failed sources.

Acceptance gate:

- After bulk import, decoded PCM remains resident; automatic decoded-buffer eviction is intentionally not part of this branch.
- Evicted tracks remain visible and become playable again after lazy re-decode.

Status: not included in `audio_memory_mitigation_2`. This branch keeps original audio bytes out of Zustand where possible and can rehydrate metadata-only sources, but it does not automatically evict decoded buffers.
