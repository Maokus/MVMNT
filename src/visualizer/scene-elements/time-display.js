...
export class TimeDisplayElement extends SceneElement {
    constructor(id = 'timeDisplay', position = 'bottomLeft', showProgress = true, config = {}, timingManager = null) {
        super('timeDisplay', id, { position, showProgress, ...config });
        this.position = position; // 'bottomLeft', 'topLeft', 'topRight', 'bottomRight'
        this.showProgress = showProgress; // Show progress bars for tick/beat
        this.fontFamily = 'Arial'; // Font family for time display
        this.fontWeight = '400'; // Font weight for time display
        this.textColor = '#FFFFFF'; // Main text color
        this.textSecondaryColor = 'rgba(255, 255, 255, 0.9)'; // Secondary text color
    ...
    buildRenderObjects(config, targetTime) {
        ...
        const barBeatTick = this.timingManager.timeToBarBeatTick(targetTime);
        ...
        const minSecMs = { minutes, seconds, milliseconds };
