...
export class ImageSequenceGenerator {
    ...
    async generateImageSequence(options = {}) {
        const {
            fps = 30,
            width = 1500,
            height = 1500,
        ...
        try {
            ...
            const limitedFrames = Math.min(totalFrames, maxFrames);

            if (totalFrames > maxFrames && maxFrames !== totalFrames) {
    ...
    async renderFramesToPNG(duration, fps, totalFrames, onProgress) {
    ...
    async createZipFile(sceneName, onProgress) {
    ...
    downloadImageSequence(zipBlob, filename = 'image-sequence.zip') {
        try {
