...
const AdminTagPanel: React.FC<AdminTagPanelProps> = ({ onTagsChanged }) => {
    ...
    return (
        <div className="mb-6 rounded-lg border border-neutral-700 bg-neutral-900/50">
            ...
            >
                <span>
                    Tag Management <span className="text-neutral-500 font-normal text-xs">({tags.length} tags)</span>
                </span>
                                    ...
                                    {renameId === tag.id ? (
                                        <>
                                            <input
                                                ...
                                                onKeyDown={(e) => {
                                                    if (e.key === 'Enter' && TAG_REGEX.test(renameName))
                                                        handleRename(tag.id);
                                        ...
                                        <>
                                            <span className="text-xs text-neutral-400 flex-1">
                                                Merge <span className="font-mono text-neutral-200">"{tag.name}"</span>{' '}
                                                into:
                                            </span>
                                            <select
                                        ...
                                        <>
                                            <span
                                                className={`flex-1 text-xs font-mono ${
                                                    tag.is_hidden ? 'line-through text-neutral-500' : 'text-neutral-200'
                                                }`}
                                            >
                                                {tag.name}
                                                {tag.is_hidden && (
